...
static void test4_1(void);
static void test4_2(void);
...
static void test2_3(void) {
	...
	assert(RoadMapGetIslandName(map, 0) == NULL);
...
// Testing closing a one-way road
static void test4_1(void) {
	RoadMap map = RoadMapNew(4);

	RoadMapAddRoad(map, 0, 1, false, 5);
	RoadMapAddRoad(map, 0, 2, false, 5);
	RoadMapAddRoad(map, 0, 3, false, 5);

	RoadMapAddRoadClosure(map, 0, 2, timeNew(12, 0), timeNew(13, 0));

	struct road roads[MAX_ROADS_PER_NODE];

	// Checking roads from 0
	int numRoads = RoadMapGetRoadsFrom(map, 0, roads);
	assert(numRoads == 3);
	sortRoads(roads, numRoads);

	assert(roads[0].toNode == 1);
	assert(!roads[0].affectedByStorms);

	assert(roads[1].toNode == 2);
	assert(roads[1].affectedByStorms);
	assert(timeEquals(roads[1].closedFrom, timeNew(12, 0)));
	assert(timeEquals(roads[1].closedUntil, timeNew(13, 0)));

	assert(roads[2].toNode == 3);
	assert(!roads[2].affectedByStorms);	

	// Checking roads to 2
	numRoads = RoadMapGetRoadsTo(map, 2, roads);
	assert(numRoads == 1);

	assert(roads[0].fromNode == 0);
	assert(roads[0].affectedByStorms);
	assert(timeEquals(roads[0].closedFrom, timeNew(12, 0)));
	assert(timeEquals(roads[0].closedUntil, timeNew(13, 0)));
...
// Testing closing a two-way road
static void test4_2(void) {
	RoadMap map = RoadMapNew(4);

	RoadMapAddRoad(map, 0, 1, true, 5);
	RoadMapAddRoad(map, 0, 2, true, 5);
	RoadMapAddRoad(map, 0, 3, true, 5);
	...
	// Checking roads from 2
	numRoads = RoadMapGetRoadsFrom(map, 2, roads);
	assert(numRoads == 1);

	assert(roads[0].toNode == 0);
	assert(roads[0].affectedByStorms);
	assert(timeEquals(roads[0].closedFrom, timeNew(12, 0)));
	assert(timeEquals(roads[0].closedUntil, timeNew(13, 0)));

	// Checking roads to 0
	numRoads = RoadMapGetRoadsTo(map, 0, roads);
	assert(numRoads == 3);
	sortRoads(roads, numRoads);

	assert(roads[1].fromNode == 2);
	assert(roads[1].affectedByStorms);
	assert(timeEquals(roads[1].closedFrom, timeNew(12, 0)));
	assert(timeEquals(roads[1].closedUntil, timeNew(13, 0)));
