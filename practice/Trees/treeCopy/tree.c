...
static void treeReadPrompt(char *traversal, int treeNo);
...
static struct node *reconstruct(int *preorder, int *inorder, int size);
static int indexOf(int *array, int size, int elem);
static struct node *newTreeNode(int value);

static int *mergeSortCopy(int *array, int arraySize);
static void mergeSort(int *array, int arraySize);
static void doMergeSort(int *array, int lo, int hi);
...
static struct node *reconstruct(int *preorder, int *inorder, int size) {
	...
	struct node *t = newTreeNode(preorder[0]);
...
// Prints ascii tree for given struct node *structure
...
static void doMergeSort(int *array, int lo, int hi) {
	if (hi - lo <= 0) return;
	int mid = (lo + hi) / 2;
	doMergeSort(array, lo, mid);
	doMergeSort(array, mid + 1, hi);
