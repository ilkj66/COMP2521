...
struct node *listHelp(struct node *list, int n, int value, int *counter);
...
void listInsertNth(struct list *list, int n, int value) {
	// TODO
	int counter = 0;
	list->head = listHelp(list->head, n, value, &counter); 
}

struct node *listHelp(struct node *list, int n, int value, int *counter) {
	if (list == NULL || (*counter) == n) { // base case: list is empty or on the insert position
		struct node *new = newNode(value);
		new->next = list;
		return new;
	}
	(*counter)++; //counter update
	list->next = listHelp(list->next, n, value, counter);
	return list;
}
