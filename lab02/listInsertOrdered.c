...
struct node *listHelper(struct node *list, int value);
...
void listInsertOrdered(struct list *list, int value) {
	// TODO
	list->head = listHelper(list->head, value);
}

struct node *listHelper(struct node *list, int value) {
	if (list == NULL || list->value >= value) { // base case or special case
        struct node *new = newNode(value);
        new->next = list; // insert
        return new; // don't need to let previous node->next = new, it will link when recall (82 line)
    }

    list->next = listHelper(list->next, value);
    return list;
}
